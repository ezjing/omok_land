...
function Game(props) {
  ...
  useEffect(() => {
    // 좌표 클릭시 이벤트
    if (props.gaming) {
      console.log(chatt.filter(item => item.topic === "game"))
      if (coordinate !== "") {
        ...
        // 착수의 가로줄 배열
        let garo = chatt.filter(item => +item.coordinate.slice(item.coordinate.indexOf(', ') + 2) === y)
  ...
  return (
    <Box>
      <GlobalStyle/>
      <Pan coordinate={setCoordinate} gaming={props.gaming}/>
    </Box>
  )
}
